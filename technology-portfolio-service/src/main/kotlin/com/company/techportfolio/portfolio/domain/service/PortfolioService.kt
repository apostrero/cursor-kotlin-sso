...
class PortfolioService(
    ...
    fun getPortfoliosByOwner(ownerId: Long): Flux<PortfolioSummary> {
        return portfolioQueryRepository.findPortfolioSummariesByOwner(ownerId)
            .onErrorResume { error ->
                logger.error("Error retrieving portfolios for owner $ownerId: ${error.message}")
                Flux.empty()
            }
    }
        ...
        return portfolioRepository.findById(portfolioId)
            ...
            .flatMap { portfolio ->
                ...
                technologyRepository.save(technology)
                    ...
                    .flatMap { toTechnologyResponse(it) }
            }
            .onErrorMap { error ->
                when (error) {
                    is IllegalArgumentException -> error
    ...
    private fun toPortfolioResponse(portfolio: TechnologyPortfolio): Mono<PortfolioResponse> {
        logger.debug("toPortfolioResponse called with portfolio - ID: ${portfolio.id}, Name: '${portfolio.name}', Type: ${portfolio.type}")
        return technologyRepository.findByPortfolioId(portfolio.id!!)
            ...
            .map { technologies ->
                ...
                logger.debug("Created PortfolioResponse - ID: ${response.id}, Name: '${response.name}', Type: ${response.type}")
                response
            }
    }
